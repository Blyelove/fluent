    ...
    return (
      <div>
        <div className="shell" style={{ paddingBottom: 0, minHeight: 0 }}>
          <button className="btn btn-ghost" style={{ padding: 12, minHeight: 44, fontSize: 14 }} onClick={() => setView('profiel')}>
            ← Terug naar profiel
          </button>
        </div>
        <BadgesScreen />
  ...
  return (
    <div className="shell">
      ...
      {/* de held van dit scherm: je transformatie staat bovenaan, want dat is
          waar je trots op bent. Het beheer verhuisde naar onderen. */}
      {(() => {
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        <div className="glass stat-card">
          <div className="stat-value gold-text num">{totalXp(state)}</div>
          <div className="stat-label">Totaal XP</div>
        </div>
        <div className="glass stat-card">
          <div className="stat-value num">{state.streak}</div>
          <div className="stat-label">{state.streak === 1 ? 'Dag reeks' : 'Dagen reeks'}</div>
        </div>
        <div className="glass stat-card">
          <div className="stat-value num">{wordsLearned(state, state.courseId)}</div>
          <div className="stat-label">Woorden geleerd</div>
        </div>
        <div className="glass stat-card">
          <div className="stat-value num">{progress.completed.length}</div>
      ...
      <div className="glass" style={{ padding: '6px 18px', marginBottom: 28 }}>
        ...
        <div className="spread" style={{ padding: '14px 0', borderBottom: '1px solid var(--line)' }}>
          ...
          <span style={{ fontWeight: 600 }}>{'❄'.repeat(Math.max(0, state.freezes)) || 'geen'}</span>
        </div>
        <div className="spread" style={{ padding: '14px 0' }}>
          <span style={{ fontWeight: 500 }}>Doelen gehaald</span>
          <span className="gold-text num" style={{ fontWeight: 700 }}>{state.goalsDone.length}</span>
        </div>
      </div>
        ...
        {state.currentUser && state.accounts[state.currentUser] && (
          <>
            ...
            <div style={{ padding: '12px 0', borderBottom: '1px solid var(--line)' }}>
              <button className="btn btn-ghost" style={{ padding: 12, minHeight: 44, fontSize: 14 }} onClick={state.logout}>
                Uitloggen
              </button>
            </div>
          </>
