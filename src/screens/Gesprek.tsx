...
import { GESPREKKEN, type GesprekAntwoord, type GesprekScenario } from '../content/gesprekken'
 ...
 * de microfoon gebruikt oefent uitspraak, wie tikt oefent lezen — allebei
...
export function GesprekkenScreen({ onExit }: { onExit: () => void }) {
  ...
  const course = courses[courseId]
  const vlag = courseFlagCode[courseId]
  const scenarios = GESPREKKEN[courseId] ?? []
  const [actief, setActief] = useState<GesprekScenario | null>(null)
