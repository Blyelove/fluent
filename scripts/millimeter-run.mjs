...
try {
  ...
  for (const scherm of SCHERMEN) {
    ...
    console.log(
      `${scherm.naam.padEnd(8)} schil ${String(r.schilLinks).padStart(3)}+${r.schilBreed}  lijnen ${String(r.uitlijning.lijnen.length).padStart(2)} van ${String(r.uitlijning.blokken).padStart(2)} blokken` +
...
const totaal = uitslagen.reduce(
  (n, u) => n + Math.max(0, u.uitlijning.lijnen.length - 1) + u.ritme.buitenLadder.length + u.zonderAanraking.length + u.randfouten.length,
  0,
)
