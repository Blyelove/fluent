...
1. KIJKEN MET JE EIGEN OGEN — OP MOBIEL. Zorg dat de dev-server draait (npm run dev, poort 5199), zet het venster op 375×812 en open de app. Bijna iedereen gebruikt dit op een telefoon; een oordeel op een breed scherm is waardeloos. Klik als een échte gebruiker door minstens drie schermen en maak screenshots. Beoordeel keihard eerlijk: wat is saai, lelijk, traag, verwarrend, leeg of niet verslavend genoeg? Noteer elke bug, elke lege staat en elk moment waarop iemand zou afhaken.
   ...
   DE AUDIT DRAAIT NIET IN HET BROWSERPANEEL. Dat paneel springt terug naar zijn eigen breedte (465) en op die maat valt alles net binnen het scherm. Draai `node scripts/audit-run.mjs 375` voor de volledige meting: zes schermen maal zestien werelden, contrast, tikdoelen onder 44px, horizontale scroll en middenstreepjes. Elke combinatie wordt via `?wereld=` in de link gepind en de runner weigert het antwoord als de gemeten breedte of wereld niet klopt met wat er gevraagd is. Geef de live link mee als tweede argument om te meten wat een bezoeker écht krijgt: `node scripts/audit-run.mjs 375 https://blyelove.github.io/fluent/`.
   VINDT HIJ IETS, dan wijst `node scripts/overloop.mjs 375 <tab>` aan wélk element buiten beeld steekt, en `node scripts/waarom.mjs "<tekst>" <wereld> <tab>` welke kleur op welke stapel vlakken ligt. Los het op met een token, niet met een losse kleur op die ene plek: `--inkt-oplichten` en `--metaal-verdiepen` zeggen per wereld of hij licht of donker is, en daar hangt de rest aan.
   REGEL VOOR ELKE METING: laat hem terugvertellen onder welke condities hij gemeten heeft, en vergelijk dat met wat je vroeg. Een audit die de wereld zelf op het document zet, meet de wereld die de app er meteen weer overheen zet, en meldt dan trots dat alles schoon is.
...
- **14-08-2026, ronde 66** - **De Arena is een ervaring geworden: de klap die je voelt, een tegenstander die zich gedraagt, en vriendduels in dezelfde jas.** Daarmee is punt 2 van het doel rond, op online matchmaking na.
  ...
  **De tegenstander.** Hij had spreiding op zijn denktijd, maar geen gedrag. Nu reageert hij op de stand: voor staan laat hem versnellen, achter staan laat hem twijfelen, en op zijn laatste schild speelt hij trager én scherper, want wie op matchpoint staat gokt niet meer. Jouw tempo maakt hem grillig in plaats van gelijkmatig sneller. Wat hij voelt staat op het scherm: "aarzelt…" of "ruikt bloed". Beide in een echt potje waargenomen. De berekening staat als pure functie in `arena.ts`, los van het scherm.

  **Het schaduwduel.** Er is geen server, dus twee mensen kunnen niet tegelijk in één arena staan. Dat hoeft ook niet: je speelt je potje, elke beurt wordt opgenomen (goed of fout, en hoelang je erover deed), en die opname gaat als link naar je vriend. Bij hem staat jouw schaduw in de arena, als donker silhouet, met exact dezelfde vragen in dezelfde volgorde en jouw eigen tempo. Een gevecht dus, geen scorevergelijking. De link is 131 tekens. Online matchmaking klikt hier zo in: dan komt de opname van een server in plaats van uit een link. Getest van begin tot eind met `scripts/schaduw-proef.mjs`, dat het potje uitspeelt, de link uit het klembord afvangt, hem als de vriend opent en nakijkt of de vragen werkelijk gelijk zijn.

  **Gereedschap erbij:** `scripts/arena-proef.mjs` (speelt door tot er echt een schild sneuvelt en telt de zichtbare delen), `scripts/schaduw-proef.mjs` (de hele duellus).

- **14-08-2026, ronde 65** - **De audit meet nu wat hij denkt te meten, en alle 96 combinaties zijn schoon, ook op de live link.** Zes schermen (Leren, Spelen, Divisie, Oefenen, Profiel, Arena) maal zestien taalwerelden, op een echte 375 breed: contrast, tikdoelen, horizontale scroll en middenstreepjes allemaal nul. Niet alleen lokaal maar ook gemeten tegen blyelove.github.io/fluent zelf, dus dit is wat een bezoeker krijgt.

  **LES 1: een audit die de wereld zelf omzet, meet de verkeerde wereld.** De audit zette `data-wereld` op het document en las daarna af. Maar de app zet zijn eigen wereld daar terug zodra hij hertekent, en op de Arena en de Divisie gebeurt dat continu. Dan meet je een donkere wereld terwijl je denkt een lichte te meten, en meldt de audit vrolijk dat alles schoon is. Dat is erger dan niet meten, want je gelooft het en gaat door. Nu pint elke combinatie zichzelf via `?wereld=` in de link, geeft de meting terug wélke wereld hij gemeten heeft, en weigert de runner het antwoord als dat niet klopt met wat er gevraagd is. Diezelfde controle staat op de breedte. **Regel: laat een meting altijd terugvertellen onder welke condities hij gemeten heeft, en vergelijk dat met wat je vroeg.**

  **LES 2: een filter dat valse alarmen wegneemt, neemt ook echte weg.** De audit sloeg elk element met een emoji over, bedoeld voor elementen die alléén uit emoji bestaan. Daardoor bleef "Houten Arena" ongemeten omdat er toevallig een boompje voor stond: romig goud op een romige muur, 1,19 op 1. Nu telt alleen of er echte letters overblijven na het strippen van de emoji.

  **LES 3: het paneel in de editor is geen telefoon.** Het springt terug naar 465 breed en op die maat valt alles net binnen het scherm. Op een echte 375 bleek je op Leren, Spelen en Divisie negentig pixels opzij te kunnen schuiven. De afsnijregel stond alleen op `html`, en Chromium geeft een `clip` op het wortelelement niet door aan het venster; nu staat hij ook op `body`. De audit draait daarom niet meer in het paneel maar in een eigen headless Edge (`scripts/audit-run.mjs`), waar de breedte wordt afgedwongen en nagemeten.

  **Wat er structureel uit kwam:** twee knoppen per wereld die één ding zeggen, is deze wereld licht of donker. `--inkt-oplichten` tilt een accentkleur op als hij op de altijd donkere heldkaart valt (het donkere cyaan van een lichte wereld verdween daarin), `--metaal-verdiepen` trekt een vast metaal naar de inkt toe op een licht vlak (brons en zilver zijn echte kleuren en staan vast). Een nieuwe lichte wereld hoeft alleen die twee te zetten in plaats van elke kleur na te lopen. Verder: `--goud-tekst` en `--goud-tekst-dim` voor goud als tekstkleur, `--ink-op-hot` en `--schaduw-op-hot` voor inkt op een hotkleur, `--ink-op-unit` voor inkt op een unitkleur.

  **Ook opgelost:** de houten en de stenen arena gebruikten emoji uit 2020, die op oudere toestellen als leeg blokje verschijnen (gezien op Windows 10); vervangen door tekens die overal bestaan. En met `?wereld=` is elke wereld nu op elk scherm vast te leggen zonder iets in te stellen, wat de bewijsfoto's in `docs/bewijs` mogelijk maakt.

  **Gereedschap dat blijft staan:** `scripts/audit-run.mjs` (de 96 metingen, met live-url als tweede argument), `scripts/audit.js` (de meting zelf), `scripts/overloop.mjs` (wélk element steekt buiten beeld), `scripts/waarom.mjs` (welke kleur op welke stapel vlakken ligt).

- **14-08-2026, ronde 64** — **De drie pijlers zijn af.** (1) De meestermantel hangt niet langer alleen als belofte in het paneel maar op je personage zelf: breder en langer dan elke cape, in de kleuren van die taalwereld, gouden zoom en een 99 op de kraag, overal zichtbaar. (2) **De proeverij** staat onder Instellingen: elf uitgewerkte richtingen (vier XP-drops met de gedurfde Regen die uiteenvalt in wegdwarrelende cijfers, vier niveaumomenten met de gedurfde Zonsopgang waarbij het scherm juist licht wordt, drie arena-opkomsten met het gedurfde Spotlicht), met een Speel-af-knop die een échte XP-boeking doet zodat je het moment ziet voordat je kiest. Geen mockups: de keuze stuurt het echte component. (3) **De zichtbare spanning in de Arena**: matchpoint met trage randgloed (rood als jij wankelt, groen als hij dat doet, en "alles of niets" als jullie allebei op één schild staan), een cyaan lichtveeg bij jouw slag en een rode terug bij zijn tegenzet, en bij de beslissing confetti óf een korte stilte waarin het scherm dooft en er bewust geen geluid komt. Met ?matchpoint=1 begin je meteen op scherp. **LES: meet nooit in een venster dat je niet hebt gecontroleerd.** Mijn punt-5-audit meldde vier te kleine tikdoelen en horizontale scroll in de Arena; het browserpaneel bleek op nul breedte te staan, dus alles was 40px breed en dus per definitie te klein. Op echte 375 breed: nul te kleine tikdoelen, nul horizontale scroll, nul te veel oneindige animaties, en de contrastaudit schoon in alle tien geteste werelden. Controleer window.innerWidth vóór elke meting; een audit die op een verkeerd venster draait liegt met veel vertoon van cijfers.
- **14-08-2026, ronde 63** — **DE ARENA: Clash of Clans in een taal-app.** Eén tegen één met drie schilden elk, elke fout breekt er één, en snel goed antwoorden drukt de foutkans van je tegenstander omhoog. Zeven arena's van Houten tot Meesterarena met elk een eigen gladiator die live voelt, bekers (+30/-10, nooit onder nul), winst geeft echte XP, verlies kost nooit voortgang. Derde tab in de speelhal, arenachip op het profiel naast het vaardighedenraster. Eromheen: het RS-blok werd afgemaakt (XP-drops op echte boekingen via één store-luisteraar, niveauviering met mijlpalen en de 92-knipoog, meestermantel als silhouetbelofte), de taalwissel verhuisde van Home naar het profiel en de wereldkiezer werd daar een instellingenrij. Screenshots draaien nu betrouwbaar via Edge headless met schaalfactor 1,3333 op venster 500x1083 (Windows-schaal plus het minimale vensterbreedte van Chromium vertekenden anders de opmaak). **TWEE LESSEN. Eén:** laat spelverloop nooit van een animatieframe afhangen. De vraagwissel wachtte op een exit-animatie; in een achtergrondtab komen animatieframes nooit, dus het gevecht bevroor. Op een telefoon die halverwege wordt weggelegd gebeurt hetzelfde. Wissel op state, animeer alleen de intrede, en doe dat met CSS. **Twee:** een verborgen tabblad krijgt van Chrome maar één timer per minuut; wie daar test jaagt op spoken. Een stille AudioContext houdt de tab wakker tijdens het testen, en het beste bewijs blijft de uitslag in de store: bekers nul naar dertig, één gevecht, één winst, exact kloppend.
